...
func move_piece(dir):
	if can_move(dir):
		...
		if dir == Vector2i.DOWN:
			...
			next_piece_atlas = Vector2i(shapes_full.find(next_piece_type) + 1, 0)
			clear_panel()
			create_piece()
			check_game_over()

func can_move(dir):
	#check if there is sapce to move
	var cm = true
	for i in active_piece:
...
func can_rotate():
	var cr = true
	var temp_rotatin_index = (rotation_index + 1) % 4
	for i in piece_type[temp_rotatin_index]:
...
func new_game():
	...
	piece_atlas = Vector2i(shapes_full.find(piece_type) + 1, 0)
	next_piece_type = pick_piece()
	next_piece_atlas = Vector2i(shapes_full.find(next_piece_type) + 1, 0)
...
func _ready():
	new_game()
	$HUD.get_node("Restart").pressed.connect(new_game)
...
func _process(delta):
	if game_running:
		...
		#apply downard movement every frame
		steps[2] += speed
		#move the piece
